...
- P2.5 的 `peak_detection.detect_peaks()` 使用全部轻量分数记录，不预先做分位数过滤。固定用 Python `round(score, 2)` 生成检测分数，不新增配置。连续检测分数相同的帧合为平台，平台必须高于两侧邻居（邻居也使用检测分数）；边界仅比较存在的一侧。代表取平台内原始最高分帧，并列时取最靠近平台下标中心的帧，距离相同时取较早帧。单帧和舍入后全等分曲线得到一个代表峰，空输入得到零个峰。时间戳必须严格递增，分数与时间戳必须有限。
- 舍入只用于找峰，不改写原始分数；Anchor 排名、扩展阈值和 Meta 继续使用原始精度。采用 Python 浮点 `round` 语义，不是分差容差：例如 0.174 和 0.176 分属 0.17 和 0.18。该规则减少微小波动形成的平台内多峰，不保证事件合并。
